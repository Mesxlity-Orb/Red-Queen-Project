...
export default function BackendManager() {
  ...
  const fetchUsers = async () => {
    try {
      const res = await apiFetch('/api/biometrics/users');
      if (res.ok) {
  ...
  const handleDelete = async (name: string) => {
    ...
    try {
      const res = await apiFetch(`/api/biometrics/users/${encodeURIComponent(name)}`, {
        method: 'DELETE'
      });
      if (res.ok) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const res = await apiFetch('/api/biometrics/register', {
        method: 'POST',
